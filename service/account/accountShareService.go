...
func (b *share) CreateUserInvitation(
	...
	return invitation, db.Transaction(
		ctx, func(ctx *cus.TxContext) (err error) {
			...
			_, err = userModel.NewDao(tx).AddFriend(
				inviter.UserId, invitee.ID, userModel.FriendAddModeOfAccountInvitation,
			)
			return
		},
	)
...
func (b *share) AddAccountUser(
	...
	return accountUser, db.Transaction(
		ctx, func(ctx *cus.TxContext) (err error) {
	...
	err = db.Transaction(
		ctx, func(ctx *cus.TxContext) (err error) {
			...
			_, _, err = logServer.RecordAccountLog(
				&mapping,
				logModel.BaseAccountLog{
					UserId:    user.ID,
					AccountId: mainAccount.ID,
					Operation: constant.LogOperationOfAdd,
				}, tx,
			)
			return
		},
...
func (b *share) DeleteAccountMapping(
	user userModel.User,
	mapping accountModel.Mapping,
	...
	return db.Transaction(
		ctx, func(ctx *cus.TxContext) error {
			...
			_, _, err = logServer.RecordAccountLog(
				...
				logModel.BaseAccountLog{
					UserId:    user.ID,
					AccountId: mapping.MainId,
					Operation: constant.LogOperationOfDelete,
				},
				tx,
			)
			if err != nil {
...
func (b *share) UpdateAccountMapping(
	...
	return newMapping, db.Transaction(
		ctx, func(ctx *cus.TxContext) error {
			tx := ctx.GetDb()
			_, _, err = logServer.RecordAccountLog(
				&mapping,
				logModel.BaseAccountLog{
					UserId:    user.ID,
					AccountId: mapping.MainId,
					Operation: constant.LogOperationOfUpdate,
				},
				tx,
			)
			if err != nil {
